...
namespace left_hand_api
{
	...
	left_hand::left_hand()
	{
		...
		for (int i = 0; i < 6; i++)
		{
			...
			errorCodes[i] = 0;
			statuses[i] = 0;
			strncpy(ipAddress, "192.168.137.39", sizeof(ipAddress) - 1);
			ipAddress[sizeof(ipAddress) - 1] = '\0';
		}
	}
	void left_hand::setIpAddress(const char *ip)
	{
	...
	const char *left_hand::getIpAddress() const
	{
		return ipAddress;
	}

	void left_hand::setRobotId(uint8_t id)
	{
		robotId = id;
	}

	uint8_t left_hand::getRobotId()
	{
		return robotId;
	}

	void left_hand::setJointCurrent(int jointNumber, uint16_t current)
	{
	...
	uint16_t left_hand::getJointCurrent(int jointNumber)
	{
		if (jointNumber >= 0 && jointNumber < 6)
		{
	...
	void left_hand::setJointAngle(int jointNumber, uint16_t angle)
	{
		if (jointNumber >= 0 && jointNumber < 6)
		{
	...
	uint16_t left_hand::getJointAngle(int jointNumber)
	{
		if (jointNumber >= 0 && jointNumber < 6)
		{
	...
	void left_hand::setJointForce(int jointNumber, uint16_t force)
	{
		if (jointNumber >= 0 && jointNumber < 6)
		{
	...
	uint16_t left_hand::getJointForce(int jointNumber)
	{
		if (jointNumber >= 0 && jointNumber < 6)
		{
	...
	void left_hand::setJointErrorCode(int jointNumber, uint8_t code)
	{
		if (jointNumber >= 0 && jointNumber < 6)
		{
	...
	uint8_t left_hand::getJointErrorCode(int jointNumber)
	{
		if (jointNumber >= 0 && jointNumber < 6)
		{
	...
	void left_hand::setJointStatus(int jointNumber, uint8_t statusInfo)
	{
		if (jointNumber >= 0 && jointNumber < 6)
		{
	...
	uint8_t left_hand::getJointStatus(int jointNumber)
	{
		if (jointNumber >= 0 && jointNumber < 6)
		{
	...
	int left_hand::left_hand_init()
	{
		...
		serverAddr.sin_addr.s_addr = inet_addr("192.168.137.39");
		std::cout << "sockfd_init:" << sockfd << endl;
		this->HandState = IDLE;

		readdata = std::thread(&left_hand::UpdateStatus, this, 0);
		return sockfd;
	}

	void left_hand::SenderThread(double time_g)
	{
	...
	void left_hand::UpdateStatus(int arg)
	{
		this->HandState = IDLE;
		while (1)
		{
			...
			for (int i = 0; i < 6; i++)
			{
				int ret = this->getJointStatus(i);
				if (ret == 0 || ret == 1)
				{
					this->HandState = RUNNING;
					break;
				}
				else if (ret == 6)
				{
					this->HandState = WARNING;
					break;
				}
				else if (ret == 7)
				{
	...
	int left_hand::Angle_Set(uint8_t ID, uint16_t val1, uint16_t val2, uint16_t val3, uint16_t val4, uint16_t val5, uint16_t val6, double time_g)
	{
		uint8_t i = 0;
	...
	int left_hand::Angle_Set(uint8_t ID, uint16_t val1, uint16_t val2, uint16_t val3, uint16_t val4, uint16_t val5, uint16_t val6, double time_g, uint16_t _force)
	{
		uint8_t i = 0;
		uint8_t m_unChecksum = 0;
		UDP_Data.m_send_array[0] = kSend_Frame_Head1;
	...
	int left_hand::Force_Set(uint8_t ID, uint16_t val1, uint16_t val2, uint16_t val3, uint16_t val4, uint16_t val5, uint16_t val6)
	{
		uint8_t i = 0;
		uint8_t m_unChecksum = 0;
	...
	int left_hand::Gesture_Force_Clb_Set(uint8_t ID)
	{
		uint8_t i = 0;
		uint8_t m_unChecksum = 0;
		UDP_Data.m_send_array[0] = kSend_Frame_Head1;
	...
	int left_hand::Speed_Set(uint8_t ID, uint16_t val1, uint16_t val2, uint16_t val3, uint16_t val4, uint16_t val5, uint16_t val6)
	{
		uint8_t i = 0;
		uint8_t m_unChecksum = 0;
		UDP_Data.m_send_array[0] = kSend_Frame_Head1;
	...
	int left_hand::Clean_error_Set(uint8_t ID)
	{
		uint8_t i = 0;
		uint8_t m_unChecksum = 0;
		UDP_Data.m_send_array[0] = kSend_Frame_Head1;
	...
	int left_hand::Action_SEQ_index(uint8_t ID, int index)
	{
		uint8_t i = 0;
		uint8_t m_unChecksum = 0;
		UDP_Data.m_send_array[0] = kSend_Frame_Head1;
	...
	int left_hand::Action_SEQ_run(uint8_t ID)
	{
		uint8_t i = 0;
		uint8_t m_unChecksum = 0;
		UDP_Data.m_send_array[0] = kSend_Frame_Head1;
	...
	int left_hand::Current_Limit_Set(uint8_t ID, uint16_t val1, uint16_t val2, uint16_t val3, uint16_t val4, uint16_t val5, uint16_t val6)
	{
		uint8_t i = 0;
		uint8_t m_unChecksum = 0;
	...
	int left_hand::Angle_Get(uint8_t ID, double time_g)
	{
		uint8_t i = 0;
		uint8_t m_unChecksum = 0;
		UDP_Data.m_send_array[0] = kSend_Frame_Head1;
	...
	int left_hand::Current_Get(uint8_t ID, double time_g)
	{
		uint8_t i = 0;
		uint8_t m_unChecksum = 0;
		UDP_Data.m_send_array[0] = kSend_Frame_Head1;
	...
	int left_hand::Force_Get(uint8_t ID, double time_g)
	{
		uint8_t i = 0;
		uint8_t m_unChecksum = 0;
		UDP_Data.m_send_array[0] = kSend_Frame_Head1;
	...
	int left_hand::Status_Get(uint8_t ID, double time_g)
	{
		uint8_t i = 0;
		uint8_t m_unChecksum = 0;
		UDP_Data.m_send_array[0] = kSend_Frame_Head1;
	...
	int left_hand::Error_Get(uint8_t ID, double time_g)
	{
		uint8_t i = 0;
		uint8_t m_unChecksum = 0;
		UDP_Data.m_send_array[0] = kSend_Frame_Head1;
	...
	int left_hand::Action_do(int act_number)
	{
	...
	int left_hand::call_count_left = 0;

	int left_hand::sin_do()
	{
	...
	int left_hand::key_do(int key_number)
	{
		std::thread sin_thread_left;
		...
		else if (key_number == 5)
		{
			...
			sin_thread_left = std::thread(&left_hand::sin_do, this);
			sin_thread_left.detach();
			// call_count_left++;
		}
		...
		else if (key_number == 70)
		{
			Angle_Set(1, 1000, 1000, 1000, 1000, 1000, 1000, 7);
			call_count_left++;
		}
		else if (key_number == 71)
		{
			Angle_Set(1, 0, 0, 0, 0, 1000, 1000, 7);
			std::this_thread::sleep_for(std::chrono::milliseconds(500));
			Angle_Set(1, 0, 0, 0, 0, 0, 0, 7);
			call_count_left++;
		}
		else if (key_number == 72)
		{
			Angle_Set(1, 0, 0, 0, 0, 1000, 1000, 7);
			call_count_left++;
		}
		else if (key_number == 73)
		{
			// Angle_Set(1, 1000, 1000, 1000, 500, 500, 100, 7);
			Angle_Set(1, 1000, 1000, 1000, 480, 120, 550, 7);
			call_count_left++;
		}
		else if (key_number == 74)
		{
			Angle_Set(1, 1000, 0, 0, 1000, 0, 0, 7);
			call_count_left++;
		}
		else
		{
