...
def set_target_pwm_all(target):
    tx_messages = struct.pack('>BBBBffffff', 0x01, 0x14, 0x00, 0x00, float(target[0]), float(target[1]), float(target[2]), float(target[3]), float(target[4]), float(target[5]))
    s.sendto(tx_messages, (dh_network, dh_port_ctrl))


def set_target_pd_pos(id, target, w):
    tx_messages = struct.pack('>BBBBff', 0x01, 0x21, 0x00, id, float(target), float(w))
    s.sendto(tx_messages, (dh_network, dh_port_ctrl))
